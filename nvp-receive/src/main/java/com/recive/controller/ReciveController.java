...
public class ReciveController {
	 ...
	 */
	@RequestMapping(value = "test2", produces = "text/xml;charset=utf-8")
	public String getAnswer(String qname, String filename, String formidSuccess,String formidFail,HttpServletRequest request) {
		...
		if (query.matches("^(\\d+)")) {
			...
			log.info("发送给小I机器人的的字符串是:"+query);
			ans = WebServiceUtil.getAnswer(query);
			anse = ans.replaceAll("<[^>]*>", "");
		}
		log.info("请求完毕");
		log.info("请求的内容是:" + anse);
		log.info("请求字符串的长度是:" + anse.length());
		//4.判断结果
		...
		if(ans.matches("^(\\d+)(.*)")){
			...
			try {
				...
				desEle="http://"+addr+":8090"+formidSuccess;
		...
		int indexOf = anse.lastIndexOf("|");
		if (indexOf==(-1)){
			anse=anse+"|";
			indexOf = anse.lastIndexOf("|");
		}
		String prompt = anse.substring(0, indexOf);
		map.put("prompt", prompt);
//		else{
//			try {
//				t1=conf.getTemplate("fail.ftl");
//				desEle="http://"+addr+":8090"+8090+formidFail;
//			} catch (IOException e1) {
//				e1.printStackTrace();
//			}
//		}
		desEle="http://"+addr+":8080"+filename+formidSuccess;
